...
import { downloadCsv, type ScanRecord } from "./lib/csv";
import {
  startCamera,
  decodeImageFile,
  decodeBase64Image,
...
export function initApp(): void {
  ...
  const themeBtn = el(
    "button",
    {
      ...
      onClick: () => {
        ...
        themeBtn.innerHTML = next === "dark" ? icons.moon : icons.sun;
        themeBtn.classList.add("icon-spin");
        setTimeout(() => themeBtn.classList.remove("icon-spin"), 500);
      },
    },
    icons[getTheme() === "dark" ? "moon" : "sun"],
  );
  const header = el("header", { class: "sr-header" }, [logo, themeBtn]);

  // ----- Hero -----
  const h1 = el("h1", {}, "Scan QR codes. Fast.");
  const subhead = el(
    "p",
    { style: { maxWidth: "620px", color: "var(--muted)", marginTop: "12px" } },
    "Continuous camera scanning, image upload, or base64 paste. Everything runs in your browser. No signup, no server.",
  );
  ...
  const scannerSection = el("section", { id: "scanner", class: "section-gap" }, [
    el("p", { class: "label", style: { marginBottom: "12px" } }, "Scanner"),
    scannerPanel,
  ]);
  ...
  const resultsHeading = el("h2", {}, ["Results ", countSpan]);
  ...
  const resultsActions = el("div", { class: "row" }, [exportBtn, clearBtn]);
  ...
  const emptyState = el(
    ...
    "Scan a code to see it here. Use the camera, upload an image, or paste base64.",
  );
  ...
  // ----- How it works (SEO/AEO content) -----
  const howList = el("ol", {}, [
    el("li", {}, "Click Start camera and allow permission. Point at QR codes one after another; each decode lands in the results list."),
    el("li", {}, "Click Upload image and choose a PNG, JPG, WebP, or GIF file containing a QR code. sweep decodes it instantly."),
    el("li", {}, "Click Paste base64 and paste a base64-encoded image string. sweep decodes the QR code from the image data."),
  ]);
  const howSection = el("section", { class: "how-it-works page-enter section-gap" }, [
    el("h2", { style: { marginBottom: "12px" } }, "How it works"),
    howList,
  ]);

  // ----- Footer -----
  const footer = el("footer", {}, [
    el("p", { class: "label" }, "sweep, batch QR scanner"),
    el("p", { style: { fontSize: "12px", color: "var(--muted)" } }, "All processing happens in your browser. No data uploaded."),
  ]);
  ...
  function openBase64(): void {
    const dialog = el("dialog", {}, [
      ...
      el("p", { style: { fontSize: "13px", color: "var(--muted)", marginBottom: "16px" } }, "Paste a base64-encoded image string. sweep decodes the QR code from it."),
    ]);
